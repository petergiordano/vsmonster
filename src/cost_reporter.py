...
def generate_cost_report(episode_data: Dict[str, Any], output_path: Path = None) -> str:
    ...
    if voice_costs:
        ...
        for speaker, cost in speaker_costs.items():
            ...
            report_lines.append(f"    {speaker}: ${cost:.4f} ({char_count:,} characters)")
        report_lines.append("")
    ...
    report_lines.extend([
        ...
        f"  Average Dialogues per Scene: {total_dialogues / len(scenes):.1f}" if scenes else "  Average Dialogues per Scene: 0",
    ...
    if total_cost > 0 and timing.get("total_duration_minutes", 0) > 0:
        chars_per_dollar = metadata.get("estimated_downstream_costs", {}).get("elevenlabs_character_count", 0) / total_cost
